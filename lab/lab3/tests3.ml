...
let findMaxKey tree =
  let rec finding subtree =
    match subtree
    with BstEmpty -> raise TreeIsEmpty |
    BstNode(otherKey, _, BstEmpty) -> otherKey |
    BstNode(_, _, rightSubtree) -> finding rightSubtree
  in finding tree
;;
    ...
    BstNode(otherKey, leftSubtree, rightSubtree) ->
      if(key < otherKey)
        ...
        then BstNode(otherKey, leftSubtree, deleting rightSubtree)
          ...
          match leftSubtree, rightSubtree
          with BstEmpty, BstEmpty -> BstEmpty |
          BstEmpty, _ -> rightSubtree |
          _, BstEmpty -> leftSubtree |
          _, _ ->
            let maxNumLeft = findMaxKey leftSubtree
            in BstNode(maxNumLeft, bstDelete leftSubtree maxNumLeft, rightSubtree)
  in deleting tree
;;
