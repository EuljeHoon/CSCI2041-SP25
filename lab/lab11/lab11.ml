...
(* EVALUATISH. What's visible outside the module EVALUATOR. *)
...
primitive "is-atom"
  (fun args env ->
    ...
    with Cons (arg, Nil) ->
        (match evaluating arg env 
         with Closure (_, _, _) | Nil | Number _ | Primitive _ | Symbol _ -> tee |
              Cons (_, _) -> Nil) |
         _ ->
          oops "is-atom's arguments are more than one") ;;
...
primitive "imply"
  (fun args env ->
    let rec implying args =
      match args 
      with Nil -> 
            oops "imply needs at least one argument" |
           Cons (arg, Nil) -> 
            evaluating arg env |
           Cons (arg, rest) ->
            (match evaluating arg env 
             with Nil -> tee |
                  _ -> implying rest)|
           _ -> oops "imply expected a proper list"
    in implying args) ;;
...
primitive "let"
  (fun args env ->
    match args 
    with Cons (var, Cons (value_exp, Cons (body, Nil))) ->
        (match var 
         with Symbol name ->
                let value = evaluating value_exp env 
                in
                let new_env = envPut name value env 
                in
                evaluating body new_env |
              _ -> 
                oops "let expected first argumen with symbol") |
         _ ->
          oops "let expected three arguments") ;;
end ;;
