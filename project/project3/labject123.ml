...
sig
  ...
  val nextThings : unit -> thing    
  val token : Scanner.token ref        
  val nextToken : unit -> unit
...
struct
  ...
  let token = 
    ref EndToken (*Most recent read token*)
  
  let nextToken() =
    token := nextToken() (*Store the next input in the token*)
  
  let initialize path = 
    initialize path; (*Initialize the scanner to read token*)
    nextToken()
...
struct 
  ...
  let rec printingThing thing =
    match thing 
    with Closure _ -> raise BadThing |
         Primitive _ -> raise BadThing |
         Nil -> printf "nil" |
         Number n -> printf "%i" n |
         Symbol s -> printf "%s" s |
         Cons (_, _) ->
...
module type Lispish =
sig
  val repl : unit -> unit (* REPLACE THIS COMMENT WITH YOUR CODE! *)
...
(* COMMAND ARGUMENTS. Call the continuation ETC on every STRING argument from
  ...
  (* REPL code *)
  let repl () =
    commandArguments (fun filename -> (* Reads files one by one *)
      try (* Catch Exception with try-with *)
        Parser.initialize filename;
        let rec loop () =
          let save = Parser.nextThing () (* Reads Lisp code save into expr *)
          in
          match save 
          with Symbol "end" -> () | (* Stop looping if the line is end *)
               _ ->
                let result = Evaluator.evaluate save (* Evaluate the line *)
                in
      ...
      with
        Evaluator.EvaluatorError s ->
          Printf.printf "%s: Error occured when evaluating %s\n" filename s | (* Error message when evaluating *)
        Parser.Can'tParse s ->
          Printf.printf "%s: Parser error %s\n" filename s | (* Error messafe when parsing *)
        Printer.BadThing ->
          Printf.printf "%s: Printer error\n" filename | (* Error message when printing *)
        _ ->
          Printf.printf "%s: Internal error\n" filename) (* Taking care of other errors *)
